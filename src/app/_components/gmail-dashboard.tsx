...
export default function GmailDashboard() {
  ...
  const user = isDemo ? { id: "demo-user-id", name: "maomao", email: "mukumaomao@gmail.com", image: null } : session?.user;
  ...
  const [extraMockMessages, setExtraMockMessages] = useState<any[]>([]);
  ...
  const mockMessages = useMemo(() => {
    return [
      ...
      // CATEGORY_PERSONAL (7 messages, 3 unread, 4 read)
      {
        ...
        payload: {
          headers: [
            ...
            { name: "To", value: "mukumaomao@gmail.com" },
      ...
      // SENT (2 messages)
      {
        ...
        payload: {
          headers: [
            { name: "From", value: "mukumaomao@gmail.com" },
  ...
  const messagesQuery = api.gmail.listMessages.useInfiniteQuery(
    { maxResults: 15, q: searchQuery || defaultQuery || undefined, refresh: true },
    {
      enabled: mounted && hasGmail && ["inbox", "starred", "sent", "spam", "trash", "overview"].includes(activeTab),
      staleTime: 900000,
  ...
  const draftsQuery = api.gmail.listDrafts.useQuery(
    ...
    { enabled: mounted && hasGmail && activeTab === "drafts", staleTime: 900000 },
  );
  ...
  const userPlanQuery = api.billing.getUserPlan.useQuery(undefined, {
    enabled: mounted && (!!session || isDemo),
  });
  ...
  const calendarQuery = api.calendar.listEvents.useQuery(
    ...
    { enabled: mounted && hasCalendar && (activeTab === "calendar" || activeTab === "overview"), refetchInterval: 30000, staleTime: 5 * 60 * 1000 },
  );
  ...
  const selectedMessage = api.gmail.getMessage.useQuery(
    ...
    { enabled: mounted && !!selectedMessageId, staleTime: 5 * 60 * 1000 },
  );
  ...
  const sendMutation = api.gmail.sendEmail.useMutation({
    onSuccess: () => {
      ...
      setActiveTab("sent");
      // Refetch messages to show the newly sent email
      utils.gmail.listMessages.invalidate();
    },
        ...
        <aside className="sidebar">
          ...
          <div className="sidebar-footer" style={{ marginTop: 'auto', paddingTop: '16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <div style={{ display: "flex", alignItems: "center", gap: "12px", padding: "8px 12px", background: "transparent", borderRadius: "var(--radius-lg)", border: "1px solid var(--border)", width: '100%' }}>
              <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'var(--text-primary)', color: 'var(--bg-deep)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 600, fontSize: '14px', flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0, flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  ...
                  {isDemo && (
                    <span style={{ background: 'var(--green-soft)', color: 'var(--green)', fontSize: '9px', fontWeight: 700, padding: '1px 4px', borderRadius: '4px', textTransform: 'uppercase', flexShrink: 0 }}>
                      Demo
                    </span>
                  )}
                </div>
